    ...
    <div class="relative" ref="dropdownRef">
        ...
        >
            ...
            <ul class="flex flex-col h-auto overflow-y-auto custom-scrollbar">
                <li v-if="items.length === 0" class="py-6 text-center text-gray-500">
                ...
                >
                    ...
                    >
                        <span class="block">
                            <span
                                class="mb-1.5 block text-theme-sm text-gray-700 dark:text-gray-200"
                            >
                                {{ n.message }}
                            </span>
...
async function fetchNotifications(){
    try{
        const url = route ? route('notifications.widget') : '/notifications/widget';
        const res = await fetch(url, { headers: { 'Accept': 'application/json' } });
        if(!res.ok) throw new Error('Failed');
        const json = await res.json();
        items.value = Array.isArray(json.data) ? json.data : [];
        notifying.value = items.value.length > 0;
    }catch(e){
        // keep silent to avoid breaking header
        items.value = [];
