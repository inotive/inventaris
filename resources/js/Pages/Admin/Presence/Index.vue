...
const detectedEmployee = ref(null);
const tempPhotoBase64 = ref("");
const tempPhotoBlob = ref(null);
const tempDistance = ref(null);
const isFaceCorrect = ref(null); // 0 = benar (confirm/auto), 1 = salah (reject → manual search)

const showManualSearch = ref(false);
const searchQuery = ref("");
const manualSelectedEmployee = ref(null);
const searchInput = ref(null);

watch(showManualSearch, (newVal) => {
    if (newVal) {
        nextTick(() => {
        ...
        searchQuery.value = "";
        manualSelectedEmployee.value = null;
    }
});
...
const qrScanning = ref(false);   // true = QR sudah di-scan, menunggu Pusher
const qrMessage = ref("");        // pesan status saat QR mode
const qrCountdown = ref(0);       // countdown sebelum QR refresh
let qrRefreshTimer = null;
let qrCountdownInterval = null;
let qrWatchdogTimer = null;       // safety timer: auto-reset jika Pusher tidak merespons

const QR_EXPIRE_SECONDS = 120; // QR berlaku 2 menit

// Computed helpers for QR countdown display
const qrFormattedTime = computed(() => {
    const s = qrCountdown.value;
    const m = Math.floor(s / 60);
    const sec = s % 60;
    return `${m}:${String(sec).padStart(2, '0')}`;
});

const qrProgressPercent = computed(() => {
    return Math.max(0, (qrCountdown.value / QR_EXPIRE_SECONDS) * 100);
});
...
const generateAttendanceUrl = () => {
    ...
    // Bersihkan watchdog jika ada
    if (qrWatchdogTimer) { clearTimeout(qrWatchdogTimer); qrWatchdogTimer = null; }

    // Countdown untuk QR expire
    qrCountdown.value = QR_EXPIRE_SECONDS;
    ...
    // Auto-refresh QR setelah expire
    if (qrRefreshTimer) clearTimeout(qrRefreshTimer);
    qrRefreshTimer = setTimeout(() => {
        if (selectedOption.value === 'qr' && !qrScanning.value) {
            generateAttendanceUrl();
        }
    }, QR_EXPIRE_SECONDS * 1000);
};

// Pause detection only (keep camera stream alive) — used when switching to QR tab
function pauseDetection() {
    ...
    // Bersihkan stability timer agar tidak ada deteksi tertunda yang masih jalan
    if (stabilityTimer) {
        clearTimeout(stabilityTimer);
        stabilityTimer = null;
    }
    stableDetection.value = { employeeId: null, distance: null, startTime: null, isStable: false };
    detectionHistory.value = [];

    clearCanvas();
}

// Resume detection (camera stream already alive) — used when switching back to camera tab
function resumeDetection() {
    if (!stream) {
        // Stream was stopped (e.g. first time), do a full start
        start();
        return;
    }
    running.value = true;
...
// Watch selectedOption to pause/resume detection without destroying camera stream
...
function detectLivenessQuality(detection) {
    ...
    // Stricter threshold for detection quality
...
const modelsUrl = import.meta.env.VITE_MODELS_URL || "/models";

const inputSize = ref(320);
...
async function encodeAllEmployees() {
    // Use multiple detector configs to maximize detection rate on reference photos
    const optionsList = [
    ...
    for (const emp of employees) {
        idx++;
        statusText.value = `Encoding ${idx}/${employees.length}: ${emp?.label ?? "-"
            }`;
        if (!emp?.photo_url) {
            console.warn(
        ...
        try {
            // Encode URL to handle spaces and special characters in filename
            // Ensure absolute path with leading /
            const rawUrl = emp.photo_url.startsWith('http') ? emp.photo_url : '/' + emp.photo_url.replace(/^\/+/, '');
            const encodedUrl = encodeURI(rawUrl);
            const img = await faceapi.fetchImage(encodedUrl);
            console.log('Fetching:', encodedUrl);

            // Try multiple detector options to maximize detection on reference photos
            let desc = null;
            ...
            if (!desc) {
                console.warn(`No face for ${emp.label} at ${emp.photo_url}`);
                continue;
            }
            labeled.push(
...
async function detect() {
    ...
    if (showConfirmModal.value || showManualSearch.value) return;
    if (detecting) return;
    detecting = true;
    ...
    try {
        ...
        for (const r of resized) {
            ...
            new faceapi.draw.DrawBox(r.detection.box, {
                label: `${label}${hasMatcher ? ` (${distance.toFixed(2)})` : ""
                    }${stabilityProgress > 0
                        ...
                        if (
                            ...
                            if (
                                ...
                                if (shouldNotifyNow(consistentEmployee)) {
                                    try {
                                        // Instead of direct notifyHit, show confirmation modal
                                        detectedEmployee.value = props.employees.find(e => {
                                            const [idPart] = String(e.label).split('|');
                                            return idPart?.trim() === consistentEmployee;
                                        });

                                        if (detectedEmployee.value) {
                                            tempPhotoBlob.value = photoBlob;
                                            tempDistance.value = distance;

                                            // Convert blob to base64 for preview
                                            const reader = new FileReader();
                                            reader.onloadend = () => {
                                                tempPhotoBase64.value = reader.result;
                                                showConfirmModal.value = true;
                                            };
                                            reader.readAsDataURL(photoBlob);

                                            // Stop detection while modal is open
                                            // stop();
                                        }
...
function startCountdown() {
    ...
    timerId = window.setInterval(() => {
        if (clearCountdownForm.value > 1) {
            ...
            stopCountdown();
            showAbsenceStatuses.value = false;
...
async function notifyHit({ employeeId, distance, photo, isFaceCorrect = null }) {
    ...
    try {
        ...
        if (isFaceCorrect !== null && isFaceCorrect !== undefined)
            fd.append("is_face_correct", String(isFaceCorrect));
        ...
        //setiap 5 detik akan clear notifyData
        setTimeout(() => {
            notifyData.value.type = null;
        }, 5000);
...
onMounted(() => {
    ...
    channel.bind("attendance-barcode-event-" + props.branchId, function (data) {
        console.log("Attendance event received:", data);

        // Tandai QR sedang diproses → tampilkan spinner di QR panel
        qrScanning.value = true;
        qrMessage.value = "Memproses absensi...";

        // Watchdog: jika Pusher tidak merespons dalam 8 detik, auto-reset dan buat QR baru
        if (qrWatchdogTimer) clearTimeout(qrWatchdogTimer);
        qrWatchdogTimer = setTimeout(() => {
            console.warn('QR watchdog triggered: Pusher tidak merespons, reset QR');
            generateAttendanceUrl();
        }, 8000);

        // Reset notifyData ke struktur awal
        notifyData.value = {
            message: "",
            data: null,
            type: null,
        ...
        // Gunakan nextTick untuk memastikan reset diproses terlebih dahulu
        nextTick(() => {
            // Bersihkan watchdog karena Pusher sudah merespons
            if (qrWatchdogTimer) { clearTimeout(qrWatchdogTimer); qrWatchdogTimer = null; }

            // Set data baru setelah reset selesai
            notifyData.value = data;
            showAbsenceStatuses.value = true;

            // Langsung hilangkan spinner & pesan agar QR kelihatan lagi
            qrScanning.value = false;
            qrMessage.value = data?.data?.status || data?.message || "";

            // Jika DITOLAK (type=4): langsung generate QR baru tanpa delay
            // Jika berhasil: tunggu 3 detik agar karyawan sempat baca status
            const isDitolak = data?.data?.type === 4;
            if (isDitolak) {
                generateAttendanceUrl();
            } else {
                setTimeout(() => {
        ...
        //reset type data after 5 seconds
        // setTimeout(() => {
        //     notifyData.value.type = null;
        // }, 5000);
    });
...
const confirmDetection = async () => {
    if (!detectedEmployee.value || !tempPhotoBlob.value) return;

    try {
        const res = await notifyHit({
            employeeId: detectedEmployee.value.id,
            distance: tempDistance.value,
            photo: tempPhotoBlob.value,
            isFaceCorrect: 0, // Wajah benar — dikonfirmasi user atau auto timer
        });
        ...
        tempPhotoBlob.value = null;
        tempPhotoBase64.value = "";
    } catch (e) {
        notifyData.value = {
            message: String(e?.message || e),
            data: null,
...
const rejectDetection = () => {
    stopConfirmTimer();
    showConfirmModal.value = false;
    // Simpan tempPhotoBlob & detectedEmployee agar bisa dipakai oleh submitManualPresence
    // Jangan null-kan dulu, akan di-reset setelah submitManualPresence selesai
    showManualSearch.value = true;
};

const handleManualSelection = (employee) => {
    manualSelectedEmployee.value = employee;
};

const submitManualPresence = async () => {
    if (!manualSelectedEmployee.value) return;

    // Gunakan foto yang sudah diambil saat deteksi wajah (dari rejectDetection)
    // Jika tidak ada (manual tanpa deteksi sebelumnya), capture baru dari kamera
    let photoBlob = tempPhotoBlob.value || null;
    if (!photoBlob) {
        try {
            const canvas = document.createElement("canvas");
            const context = canvas.getContext("2d");
            ...
            console.error("Failed to capture manual photo:", e);
    ...
    try {
        const res = await notifyHit({
            employeeId: manualSelectedEmployee.value.id,
            distance: tempDistance.value ?? 0,
            photo: photoBlob,
            isFaceCorrect: 1, // Wajah salah — user pilih manual
        });
        ...
        manualSelectedEmployee.value = null;
        searchQuery.value = "";
        // Reset temp state setelah submit berhasil
        detectedEmployee.value = null;
        tempPhotoBlob.value = null;
        tempPhotoBase64.value = "";
        tempDistance.value = null;
    } catch (e) {
        notifyData.value = {
            message: String(e?.message || e),
            data: null,
...
watch(searchQuery, (query) => {
    ...
    filteredEmployees.value = props.employees.filter(e =>
...
<template>
    ...
    <div class="fixed z-50 space-y-2 top-4 right-4">
        <div v-for="alert in attendanceAlerts" :key="alert.id" :class="{
            'bg-green-500': alert.variant === 'success',
            'bg-blue-500': alert.variant === 'info',
            'bg-red-500': alert.variant === 'error',
        }" class="max-w-md px-6 py-4 text-white rounded-lg shadow-lg">
            <div v-if="alert.employeeName" class="flex items-center">
                <svg v-if="alert.type === 'masuk'" class="w-6 h-6 mr-3" fill="none" stroke="currentColor"
                    viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <svg v-else-if="alert.type === 'keluar'" class="w-6 h-6 mr-3" fill="none" stroke="currentColor"
                    viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <button @click="removeAlert(alert.id)" class="ml-4 text-white hover:text-gray-200">
    ...
    <div class="relative flex h-screen overflow-hidden">
        ...
        <div v-if="showConfirmModal"
            ...
            <div
                class="bg-white/95 backdrop-blur-2xl rounded-[2.5rem] shadow-[0_25px_50px_-12px_rgba(0,0,0,0.25)] w-full max-w-sm overflow-hidden animate-in fade-in zoom-in-95 duration-500 border border-white/40">
                <div class="p-8 text-center uppercase">
                    <div class="mb-6">
                        <h3 class="mb-2 text-3xl font-black tracking-tight text-gray-900">
                            Halo, <span class="text-blue-600">{{ detectedEmployee?.name?.split(' ')[0] }}</span>
                        </h3>
                        <p class="font-medium text-gray-500 normal-case">Apakah benar ini Anda?</p>
                    </div>

                    <div class="relative w-56 h-56 mx-auto mb-8">
                        <!-- Animated rings -->
                        <div class="absolute inset-0 border-4 rounded-full border-blue-500/10 animate-ping"></div>
                        <div class="absolute inset-[-12px] rounded-full border-2 border-blue-500/5 animate-pulse"></div>

                        <div
                            class="relative w-full h-full overflow-hidden border-4 border-white rounded-full shadow-2xl">
                            <img :src="tempPhotoBase64" class="object-cover w-full h-full" alt="Captured Face" />
                        </div>

                        <!-- Status Badge -->
                        <div
                            class="absolute -bottom-2 right-4 bg-green-500 text-white px-3 py-1 rounded-full text-[10px] font-bold shadow-lg border-2 border-white flex items-center gap-1 tracking-wider">
                    ...
                    <div class="flex flex-col gap-4">
                        <button @click="confirmDetection"
                            class="relative w-full py-5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white rounded-2xl font-bold text-xl transition-all active:scale-[0.98] shadow-xl shadow-blue-500/30 overflow-hidden flex flex-col items-center justify-center">
                            <div
                                class="relative z-10 flex items-center gap-2 transition-transform group-hover:scale-105">
                                <span class="tracking-wide uppercase">Ya, Ini Saya</span>
                                <span
                                    class="text-[10px] bg-white/20 px-1.5 py-0.5 rounded border border-white/30 font-mono">Enter</span>
                            </div>
                            <span class="relative z-10 text-[10px] font-medium opacity-80 mt-1 normal-case">
                                Konfirmasi otomatis dalam {{ confirmCountdown }} detik
                            </span>

                            <!-- Integrated Progress Bar -->
                            <div class="absolute bottom-0 left-0 w-full h-1.5 bg-black/20">
                                <div class="h-full transition-all duration-1000 ease-linear bg-white/40"
                                    :style="{ width: (confirmCountdown / 10 * 100) + '%' }"></div>
                            </div>
                        </button>

                        <button @click="rejectDetection"
                            class="flex items-center justify-center w-full gap-2 py-3 text-lg font-semibold text-gray-400 normal-case transition-all hover:text-gray-600 active:scale-95 group">
                            <span>Bukan Saya</span>
                            <span
                                class="text-[10px] bg-gray-100 px-1.5 py-0.5 rounded border border-gray-200 font-mono text-gray-400 group-hover:bg-gray-200 transition-colors">Esc</span>
                        </button>
                    </div>
                </div>
            </div>
        ...
        <div v-if="showManualSearch"
            ...
            <div
                class="bg-white/95 backdrop-blur-2xl rounded-[2.5rem] shadow-[0_25px_50px_-12px_rgba(0,0,0,0.25)] w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-500 border border-white/40 flex flex-col max-h-[90vh]">
                <div class="flex flex-col h-full p-8">
                    <div class="flex items-center justify-between mb-6">
                        <div>
                            <h3 class="text-2xl font-black tracking-tight text-gray-900 uppercase">Cari Nama Anda</h3>
                            <p class="text-sm font-medium text-gray-500">Pilih nama untuk melanjutkan presensi</p>
                        </div>
                        <button @click="showManualSearch = false"
                            class="p-3 transition-colors rounded-full hover:bg-gray-100">
                    ...
                    <div class="relative mb-6">
                        <input ref="searchInput" v-model="searchQuery" type="text"
                            class="w-full py-5 pr-6 text-lg font-bold transition-all border-0 pl-14 bg-gray-100/50 rounded-2xl focus:ring-2 focus:ring-blue-500 placeholder:font-medium"
                            placeholder="Ketik nama Anda di sini..." />
                        <svg class="absolute -translate-y-1/2 left-5 top-1/2 w-7 h-7 text-blue-500/50" fill="none"
                    ...
                    <div class="flex-1 pr-2 mb-6 space-y-3 overflow-y-auto custom-scrollbar">
                        <div v-if="filteredEmployees.length === 0 && searchQuery" class="py-12 text-center">
                            <div
                                class="flex items-center justify-center w-20 h-20 mx-auto mb-4 bg-gray-100 border-4 border-white rounded-full shadow-inner">
                            ...
                            <p class="text-lg font-bold tracking-wider text-gray-400 uppercase">Nama tidak ditemukan</p>
                        </div>

                        <div v-for="emp in filteredEmployees" :key="emp.id" @click="handleManualSelection(emp)"
                            :class="['p-5 rounded-3xl cursor-pointer flex items-center gap-5 transition-all border-2',
                                manualSelectedEmployee?.id === emp.id ? 'bg-blue-600 border-blue-600 shadow-xl shadow-blue-500/20' : 'bg-white border-transparent hover:bg-gray-50/80 hover:border-gray-100']">
                            <div
                                class="flex-shrink-0 w-16 h-16 overflow-hidden bg-gray-100 shadow-sm rounded-2xl ring-4 ring-white">
                                <img :src="emp.photo_url ? '/' + emp.photo_url.replace(/^\/+/, '') : null" class="object-cover w-full h-full" :alt="emp.name" />
                            </div>
                            <div class="flex-1">
                                <p
                                    :class="['font-black text-lg leading-tight uppercase tracking-tight', manualSelectedEmployee?.id === emp.id ? 'text-white' : 'text-gray-900']">
                                    {{ emp.name }}</p>
                                <p
                                    :class="['text-xs font-bold uppercase tracking-widest mt-0.5', manualSelectedEmployee?.id === emp.id ? 'text-blue-100' : 'text-gray-400']">
                                    {{ emp.role?.name || 'Staff' }}</p>
                            </div>
                            <div v-if="manualSelectedEmployee?.id === emp.id"
                                class="flex items-center justify-center w-8 h-8 border rounded-full bg-white/20 backdrop-blur-md border-white/30">
                    ...
                    <button v-if="manualSelectedEmployee" @click="submitManualPresence"
                        class="w-full py-5 bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white rounded-[2rem] font-black text-xl transition-all active:scale-[0.98] shadow-2xl shadow-green-500/30 flex items-center justify-center gap-4 uppercase tracking-wider">
                    ...
                    <p v-if="manualSelectedEmployee"
                        class="text-center text-[10px] text-gray-400 font-bold uppercase mt-4 tracking-[0.2em]">
                        Kamera akan otomatis menangkap wajah Anda
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-[#112D5A] w-1/2">
            ...
            <div class="flex justify-center pt-6 pb-4">
                <div class="inline-flex bg-[#1a3a6a] rounded-full p-1">
                    <button @click="selectedOption = 'camera'"
                        :class="[
                    ...
                    <button @click="selectedOption = 'qr'"
                        :class="[
                            'flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-semibold transition-all duration-200',
                            selectedOption === 'qr'
                                ? 'bg-blue-500 text-white shadow-lg shadow-blue-500/30'
            ...
            <!-- Perhatian Penting Alert Banner -->
            <div v-show="selectedOption === 'camera'" class="flex justify-center px-6 mb-2">
                <div class="relative flex items-start w-3/4 gap-3 px-4 py-3 border rounded-xl bg-amber-500/15 border-amber-400/40">
                    <!-- Pulse ring -->
                    <span class="flex-shrink-0 flex h-5 w-5 mt-0.5">
                        <span class="absolute inline-flex w-5 h-5 rounded-full opacity-50 animate-ping bg-amber-400"></span>
                        <span class="relative inline-flex items-center justify-center w-5 h-5 rounded-full bg-amber-400/20">
                    ...
                    <div>
                        <p class="text-sm font-bold text-amber-300">Perhatian Penting!</p>
                        <p class="text-xs text-amber-200/80 mt-0.5 leading-relaxed">Pastikan wajah kamu benar! Ada <strong>10 detik konfirmasi</strong> setelah wajah terdeteksi sebelum absensi tercatat.</p>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-col items-center justify-center flex-1 pb-8">
                ...
                <div class="flex flex-col items-center w-3/4 text-center"
                    v-show="selectedOption === 'camera'">
                    <div v-if="!isCameraOn" class="w-full bg-[#0d2240] rounded-2xl p-12 relative">
                        <!-- Dashed border overlay -->
                        <div class="absolute border-2 border-dashed pointer-events-none inset-4 border-blue-400/30 rounded-xl"></div>
                        <div class="flex flex-col items-center justify-center py-8">
                            ...
                            <p class="mt-1 text-sm text-blue-300/60">
                                Nyalakan kamera untuk melanjutkan proses absensi
                            </p>
                        </div>
                    </div>
                    <div class="overflow-hidden relative w-full bg-[#0d2240] rounded-2xl" v-if="isCameraOn">
                        <!-- Dashed border overlay -->
                        <div class="absolute z-10 border-2 border-dashed pointer-events-none inset-3 border-blue-400/30 rounded-xl"></div>
                        <video ref="videoRef" autoplay playsinline muted class="block w-full h-auto rounded-2xl"
                            @loadedmetadata="resizeCanvas" />
                        <canvas ref="canvasRef" class="absolute inset-0 w-full h-full pointer-events-none"></canvas>
                    </div>
                </div>
                <div class="flex flex-col items-start w-3/4 gap-3 mt-5" v-if="selectedOption === 'camera'">
                    <div v-for="(text, index) in textList" :key="index" class="flex items-start gap-2.5">
                        <div class="w-5 h-5 rounded-full bg-green-500/20 flex items-center justify-center flex-shrink-0 mt-0.5">
                            <div class="w-2.5 h-2.5 rounded-full bg-green-400"></div>
                        </div>
                        <span class="text-base leading-relaxed text-blue-100/80">{{ text }}</span>
                    </div>
                </div>

                <!-- QR Code Section -->
                <div class="flex flex-col items-center w-3/4" v-show="selectedOption === 'qr'">
                    <!-- QR Card -->
                    <div class="w-full bg-[#0d2240] rounded-2xl p-8 relative">
                        <!-- Dashed border overlay -->
                        <div class="absolute border-2 border-dashed pointer-events-none inset-4 border-blue-400/30 rounded-xl"></div>
                        <div class="relative z-10 flex flex-col items-center justify-center py-4">
                            <!-- Overlay spinner saat QR sedang diproses -->
                            <div v-if="qrScanning" class="absolute inset-0 flex flex-col items-center justify-center bg-[#0d2240]/90 rounded-xl z-20">
                                <svg class="w-10 h-10 mb-3 text-blue-400 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"></path>
                                </svg>
                                <span class="text-sm font-semibold text-blue-300">Memproses absensi...</span>
                            </div>
                            <!-- QR Icon placeholder -->
                            <div class="p-4 bg-white shadow-lg rounded-2xl">
                                <vue-qrcode :value="attendanceUrl" :options="{ width: 280, margin: 1 }"></vue-qrcode>
                            </div>
                            <!-- Label -->
                            <p class="mt-5 text-base font-medium text-blue-200/80">Arahkan QR Code ke kamera</p>
                            <!-- QR Countdown Bar -->
                            <div class="w-full px-2 mt-4" v-if="!qrScanning">
                                <div class="flex items-center justify-between mb-1.5">
                                    <span class="text-xs text-blue-300/70">QR berlaku</span>
                                    <span :class="qrCountdown <= 30 ? 'text-red-400 font-bold' : 'text-blue-200 font-semibold'" class="text-sm tabular-nums">{{ qrFormattedTime }}</span>
                                </div>
                                <div class="w-full h-1.5 bg-white/10 rounded-full overflow-hidden">
                                    <div class="h-full transition-all duration-1000 rounded-full"
                                        :class="qrCountdown <= 30 ? 'bg-red-400' : qrCountdown <= 60 ? 'bg-amber-400' : 'bg-green-400'"
                                        :style="{ width: qrProgressPercent + '%' }">
                                    </div>
                                </div>
                                <button @click="generateAttendanceUrl()" class="flex items-center justify-center w-full gap-2 py-2 mt-3 text-xs transition-all duration-200 border rounded-lg bg-white/5 hover:bg-white/10 border-white/10 text-blue-200/70 hover:text-blue-100">
                    ...
                    <!-- Status setelah QR di-scan -->
                    <div v-if="qrMessage" class="w-full px-4 py-3 mt-4 text-sm font-semibold text-center rounded-xl"
                        :class="{
                            'bg-green-500/20 text-green-300 border border-green-500/30': notifyData?.data?.type == 1,
                            'bg-yellow-500/20 text-yellow-300 border border-yellow-500/30': notifyData?.data?.type == 3,
                            'bg-red-500/20 text-red-300 border border-red-500/30': notifyData?.data?.type == 4 || notifyData?.data?.type == 5,
                            'bg-blue-500/20 text-blue-300 border border-blue-500/30': !notifyData?.data?.type,
                        }">
                        {{ qrMessage }}
                    </div>

                    <!-- QR Tips -->
                    <div class="flex flex-col items-start w-full gap-3 mt-5">
                        <div class="flex items-start gap-2.5">
                            <div class="w-5 h-5 rounded-full bg-green-500/20 flex items-center justify-center flex-shrink-0 mt-0.5">
                                <div class="w-2.5 h-2.5 rounded-full bg-green-400"></div>
                            </div>
                            <span class="text-base leading-relaxed text-blue-100/80">Tunjukkan QR Code karyawan Anda</span>
                        </div>
                        <div class="flex items-start gap-2.5">
                            <div class="w-5 h-5 rounded-full bg-green-500/20 flex items-center justify-center flex-shrink-0 mt-0.5">
                                <div class="w-2.5 h-2.5 rounded-full bg-green-400"></div>
                            </div>
                            <span class="text-base leading-relaxed text-blue-100/80">Pastikan QR Code terlihat jelas di kamera</span>
                        </div>
                        <div class="flex items-start gap-2.5">
                            <div class="w-5 h-5 rounded-full bg-green-500/20 flex items-center justify-center flex-shrink-0 mt-0.5">
                                <div class="w-2.5 h-2.5 rounded-full bg-green-400"></div>
                            </div>
                            <span class="text-base leading-relaxed text-blue-100/80">Tunggu hingga proses verifikasi selesai</span>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="bg-gradient-to-br from-[#f0f4f8] to-[#e2e8f0] w-1/2 p-8 flex flex-col">
            <div class="flex items-center justify-between">
                <svg width="178" height="58" viewBox="0 0 178 58" fill="none" xmlns="http://www.w3.org/2000/svg"
                    xmlns:xlink="http://www.w3.org/1999/xlink">
                    <rect width="58" height="58" rx="24" fill="url(#pattern0_6102_60519)" />
                    <path
                    ...
                    <path
                        ...
                        fill="#4B5675" />
                    <defs>
                        <pattern id="pattern0_6102_60519" patternContentUnits="objectBoundingBox" width="1" height="1">
                            <use xlink:href="#image0_6102_60519" transform="scale(0.002)" />
                        </pattern>
                        <image id="image0_6102_60519" width="500" height="500" preserveAspectRatio="none"
                ...
                <div class="flex flex-col items-end">
                    <span class="text-3xl font-bold text-[#1B84FF] tracking-tight">{{
                        currentTime
                    }}</span>
            ...
            <div class="flex flex-col gap-3 py-6">
                <h1 class="text-3xl text-[#1B84FF] font-bold">
            ...
            <div class="p-6 mt-4 bg-white border border-gray-200 shadow-sm rounded-2xl">
                <h2 class="pb-4 text-xl font-bold text-gray-800 border-b border-gray-200">
                    Informasi Personal Karyawan
                </h2>
                <div class="mt-2 divide-y divide-gray-100">
                    <div class="flex items-center justify-between py-4">
                        <span class="text-lg text-gray-500">Nama</span>
                        <span class="text-lg font-semibold text-gray-800">
                            {{ notifyData?.data?.employee?.name || "-" }}
                        </span>
                    </div>
                    <div class="flex items-center justify-between py-4">
                        <span class="text-lg text-gray-500">NIK</span>
                        <span class="text-lg font-semibold text-gray-800">
                            {{ notifyData?.data?.employee?.nik || "-" }}
                        </span>
                    </div>
                    <div class="flex items-center justify-between py-4">
                        <span class="text-lg text-gray-500">Departemen</span>
                        <span class="text-lg font-semibold text-gray-800">
                            {{ notifyData?.data?.employee?.department?.name || "-" }}
                        </span>
                    </div>
                    <div class="flex items-center justify-between py-4">
                        <span class="text-lg text-gray-500">Jabatan</span>
                        <span class="text-lg font-semibold text-gray-800">
                            {{ notifyData?.data?.employee?.position?.name || "-" }}
                        </span>
                    </div>
                    <div class="flex items-center justify-between py-4">
                        <span class="text-lg text-gray-500">Waktu Absen</span>
                        <span class="text-lg font-semibold text-gray-800">
                            {{ notifyData?.data?.dateTime || "-" }}
                        </span>
                    </div>
                    <div class="flex items-center justify-between py-4">
                        <span class="text-lg text-gray-500">Status Absen</span>
                        <span class="text-lg font-semibold text-gray-800">
            ...
            <div class="mt-4" v-if="showAbsenceStatuses">
                <AbsenceStatuses :type="notifyData?.data?.type" :title="notifyData?.data?.status"
