<template>
    ...
    <div class="flex overflow-hidden flex-col gap-3 px-3 h-full">
        <div class="flex justify-between items-center h-10">
            ...
            <div>
                <Link
                    :href="route('good-receipts.create')"
        ...
        >
            ...
            <div class="overflow-auto overflow-x-auto" data-simplebar>
                <table class="min-w-full text-sm table-fixed">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th
                                class="py-2.5 bg-gray-100 border-gray-200 border-y dark:border-gray-600 dark:bg-gray-800"
                            >
                                <div class="px-3 font-medium text-gray-600 dark:text-gray-300">No. GR</div>
                            </th>
                            <th
                                class="py-2.5 bg-gray-100 border-gray-200 border-y dark:border-gray-600 dark:bg-gray-800"
                            >
                                <div class="px-3 font-medium text-gray-600 dark:text-gray-300">Referensi (PO)</div>
                            </th>
                            <th
                                class="py-2.5 bg-gray-100 border-gray-200 border-y dark:border-gray-600 dark:bg-gray-800"
                            >
                            ...
                            <th
                                class="py-2.5 bg-gray-100 border-gray-200 border-y dark:border-gray-600 dark:bg-gray-800"
                            >
                                <div class="px-3 font-medium text-gray-600 dark:text-gray-300">Penerima</div>
                            </th>
                            <th
                                class="py-2.5 bg-gray-100 border-gray-200 border-y dark:border-gray-600 dark:bg-gray-800"
                            >
                            ...
                            <th class="py-2.5 bg-gray-100 border-gray-200 border-y dark:border-gray-600 dark:bg-gray-800">
                                <div class="px-3 font-medium text-gray-600 dark:text-gray-300">Total Item</div>
                            </th>
                            <th
                                class="py-2.5 bg-gray-100 border-gray-200 border-y dark:border-gray-600 dark:bg-gray-800"
                            >
                        ...
                        <tr>
                            ...
                            <th
                                class="px-2 py-2 bg-gray-50 border-gray-200 border-y dark:border-gray-600 dark:bg-gray-900"
                            >
                                <input v-model="filtersLocal.q" type="text" placeholder="Cari nomor / vendor" class="px-3 w-full h-10 text-xs rounded border-gray-300 sm:text-sm dark:border-gray-700 dark:bg-gray-800 dark:text-white/90" />
                            </th>
                            ...
                            >
                                <select v-model="filtersLocal.vendor_id" class="px-3 w-full h-10 text-xs rounded border-gray-300 sm:text-sm dark:border-gray-700 dark:bg-gray-800 dark:text-white/90">
                    ...
                    <tbody>
                        <tr v-if="!receives.data || receives.data.length === 0">
                            ...
                            >
                                ...
                                <template v-if="!isCollapsed(gkey)">
                                    ...
                                    >
                                        ...
                                        <td class="px-3 py-3">{{ row.from_po?.number || '-' }}</td>
                                        <td class="px-3 py-3">{{ row.vendor?.name || '-' }}</td>
                                        <td class="px-3 py-3">{{ row.receiver || '-' }}</td>
                                        <td class="px-3 py-3">
                                            {{ row.date }}
                                        </td>
                                        <td class="px-3 py-3">
                                        ...
                                        <td class="px-3 py-3">{{ row.total_items }}</td>
                                        <td class="px-3 py-3">
                                            <div
                                                class="flex gap-2 items-center"
                                            >
                                                <Link
                                                    :href="route('good-receipts.show', row.id)"
                                                    class="text-primary-600 hover:underline"
                                                    >Lihat</Link
                                                >
                                                <Link
                                                    :href="route('good-receipts.edit', row.id)"
                            ...
                            >
                                ...
                                <td class="px-3 py-3">
                                    <div class="flex gap-2 items-center">
                                        <Link :href="route('good-receipts.show', row.id)" class="text-primary-600 hover:underline">Lihat</Link>
                                        <Link :href="route('good-receipts.edit', row.id)" class="inline-flex items-center px-2 py-1 text-xs text-gray-700 rounded border border-gray-300 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300">Edit</Link>
                                    </div>
                                </td>
                            </tr>
                        </template>
            ...
            <div class="flex justify-between items-center px-8 py-3">
                <div class="text-xs text-gray-500">Total: {{ receives.total }}</div>
                <Pagination :pagination="receives" />
            </div>
        </div>
    </div>
</template>
...
const props = defineProps({
    receives: { type: Object, required: true },
...
const serialMap = computed(() => {
    const map = {};
    const base =
        ((props.receives.current_page || 1) - 1) *
        (props.receives.per_page || 20);
    const arr = props.receives.data || [];
    arr.forEach((row, idx) => {
...
const groupedData = computed(() => {
    ...
    const data = props.receives.data || [];
    const groups = {};
...
function badgeClass(s) {
    const map = {
        ...
        submitted: 'bg-yellow-100 text-yellow-800 dark:bg-yellow-500/20 dark:text-yellow-300',
...
function onDelete(row) {
    ...
    router.delete(route('good-receipts.destroy', row.id), {
        preserveScroll: true,
        onSuccess: () => {
