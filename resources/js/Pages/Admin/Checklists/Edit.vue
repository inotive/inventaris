<template>
    <Head :title="`Edit Checklist - ${form.name || ''}`" />
    <Toast />
            ...
            <div class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                    <div>
                        ...
                        />
                        <p
                            v-if="validationErrors.name || form.errors.name"
                    ...
                    <div>
                        ...
                        <input
                            ...
                            :class="{
                                'border-red-500':
                                    validationErrors.sop_code ||
                                    form.errors.sop_code,
                            }"
                            type="text"
                        ...
                        >
                            <template #item="{ item }">
                                ...
                                <span
                                    v-if="
                                        item.branch?.name ||
                                        item.employees_count !== undefined
                                    "
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        <div>
                            ...
                            <p class="mt-1 text-xs text-gray-500">
                                Kirim notifikasi reminder secara otomatis ke
                                karyawan
                            </p>
                        </div>
                    </div>
                    ...
                    >
                        <div>
                            <label
                                for="reminder_time"
                                class="block mb-1 text-sm text-gray-600"
                                >Jam Pengiriman Reminder (WITA)</label
                            >
                            <input
                                id="reminder_time"
                                class="px-3 py-2 w-full text-sm rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                :class="{
                                    'border-red-500': form.errors.reminder_time,
                                }"
                                type="time"
                        ...
                        <div>
                            <label
                                for="reminder_frequency"
                                class="block mb-1 text-sm text-gray-600"
                                >Frekuensi Pengiriman</label
                            >
                            <select
                                id="reminder_frequency"
                                v-model="form.reminder_frequency"
                                class="w-full text-sm font-medium placeholder-gray-500 text-gray-600 rounded-lg border-gray-400"
                                :class="{
                                    'border-red-500':
                            ...
                            <p
                                v-if="form.errors.reminder_frequency"
                                class="mt-1 text-xs text-red-500"
                            >
                                {{ form.errors.reminder_frequency }}
                            </p>
                        ...
                        <div
                            v-if="form.reminder_frequency === 'weekly'"
                            class="md:col-span-2"
                        >
                            <label class="block mb-2 text-sm text-gray-600"
                                >Hari Pengiriman Reminder</label
                            >
                            ...
                            <p
                                v-if="form.errors.reminder_days"
                                class="mt-1 text-xs text-red-500"
                            >
                                    ...
                                    >
                                        <div class="lg:col-span-8">
                                            ...
                                            >
                                                ...
                                                />
                                                <label
                                                    class="absolute top-3 -z-10 origin-[0] -translate-y-6 scale-75 transform text-sm text-gray-500"
                                                    >Pertanyaan</label
                                                >
                                            </div>
                                        </div>
...
const breadcrumbs = [
    ...
    { label: `Edit: ${props.checklist?.name || ""}` },
];
...
const form = useForm({
    ...
    sop_code: props.checklist?.sop_code || "",
    category_id: props.checklist?.category_id || null,
    department_ids: props.checklist?.department_ids || [],
    status: props.checklist?.status || "Draft",
    description: props.checklist?.description || "",
    type: props.checklist?.type || "single",
    durasi: props.checklist?.durasi || 1,
    count: props.checklist?.count || 1,
    reminder_enabled: props.checklist?.reminder_enabled ?? true,
    reminder_time: props.checklist?.reminder_time || "15:20",
    reminder_frequency: props.checklist?.reminder_frequency || "daily",
    reminder_days: props.checklist?.reminder_days || [1, 2, 3, 4, 5],
    questions: [],
});
...
const questions = ref(
    (props.questions || []).map((q) => {
        // Ensure answer_type is valid, default to 'text' if invalid
        ...
        const answerType =
            q.answer_type && validAnswerTypes.includes(q.answer_type)
                ? q.answer_type
                : "text";
...
function submit() {
    ...
    // Validasi frontend untuk Informasi Checklist
    let hasError = false;
    ...
    if (!form.category_id) {
        validationErrors.value.category_id =
            "Kategori Checklist wajib dipilih.";
        hasError = true;
    }

    if (!form.department_ids || form.department_ids.length === 0) {
        validationErrors.value.department_ids =
            "Minimal satu departemen wajib dipilih.";
        hasError = true;
    }
    ...
    form.put(route("checklists.update", form.id), {
        onSuccess: () => {
            toast.add({
                severity: "success",
                summary: "Berhasil",
                detail: "Checklist berhasil diperbarui.",
                life: 3000,
            });
            router.visit(route("checklists.index"));
        },
