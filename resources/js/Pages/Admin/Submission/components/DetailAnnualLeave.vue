<template>
    ...
    <div v-if="isOpen" @click="goBack" class="fixed inset-0 z-[9999] bg-black bg-opacity-50 flex items-center justify-center p-4">
            <div @click.stop class="bg-white rounded-lg shadow-xl w-full" :class="modalWidthClass"
                ...
                <div class="flex items-center justify-between p-4 border-b border-gray-200">
                    <h2 class="text-xl font-bold text-gray-900">Detail Pengajuan Cuti Tahunan</h2>
                    <button @click="goBack"
                ...
                <div v-if="submission" class="p-6">
                    ...
                    <div v-if="isStatusApprovedOrRejected" class="space-y-4">
                        ...
                        <!-- Grid untuk tanggal dan kategori -->
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <!-- Tanggal Pengajuan -->
                            <div>
                            ...
                            <!-- Kategori Cuti -->
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Kategori Cuti</label>
                                <div class="p-3 bg-gray-50 rounded border">
                                    <p class="text-gray-900 text-sm">
                                        {{ submission?.data?.leave_type || 'Cuti Tahunan' }}
                                    </p>
                                </div>
                            </div>
                        </div>

                        <!-- Grid untuk tanggal mulai, selesai, dan total hari -->
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <div class="p-3 bg-gray-50 rounded border">
                                    <p class="text-gray-900 text-sm">
                                        {{ formatDisplayDate(submission?.data?.corrected_start_date || submission?.data?.start_date) }}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="p-3 bg-gray-50 rounded border">
                                    <p class="text-gray-900 text-sm">
                                        {{ formatDisplayDate(submission?.data?.corrected_end_date || submission?.data?.end_date) }}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="p-3 bg-gray-50 rounded border">
                                    <p class="text-gray-900 text-sm">
                                        {{ submission?.data?.total_days ? submission.data.total_days + ' hari' : '-' }}
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- Koreksi Tanggal Log -->
                        <div v-if="submission?.data?.corrected_at" class="rounded-lg border border-amber-200 bg-amber-50 p-3">
                            <div class="flex items-start gap-2">
                                ...
                                <div class="text-xs text-amber-800">
                                    <p class="font-semibold">Telah dilakukan koreksi</p>
                                    <p class="mt-0.5">
                                        Tanggal awal: {{ formatDisplayDate(submission.data.start_date) }} – {{ formatDisplayDate(submission.data.end_date) }}
                                    </p>
                                    <p>
                                        Dikoreksi oleh <span class="font-medium">{{ submission.data.corrected_by }}</span>
                                        pada {{ submission.data.corrected_at }}
                                    </p>
                                    <p v-if="submission.data.correction_reason" class="mt-0.5 italic">
                                        "{{ submission.data.correction_reason }}"
                                    </p>
                                </div>
                            </div>
                        </div>

                        <!-- Tombol Koreksi (hanya superadmin/HR, hanya status approved) -->
                        <div v-if="canCorrectDate && selectedStatus === 'approved'">
                            <button @click="showCorrectionForm = true"
                                class="inline-flex items-center gap-1.5 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-medium text-amber-700 transition hover:bg-amber-100">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div class="relative group">
                                    ...
                                    <div
                                        ...
                                        <div class="font-medium">{{ formatDisplayDateTime(submission?.data?.approved_at, '1 Juli 2025 - 08:00') }}</div>
                                        <div class="text-gray-300 text-xs">Oleh</div>
                                        <div class="font-medium">{{ submission?.data?.approvals?.name || 'Maharani Putri'  }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="p-3 bg-gray-50 rounded border min-h-[80px]">
                                <p class="text-gray-900 text-sm leading-relaxed">
                                    {{ submission?.data?.reason || submission?.reason || '-' }}
                                </p>
                            </div>
                        </div>
    ...
    <!-- Modal Koreksi -->
    <div v-if="showCorrectionForm" class="fixed inset-0 z-[10002] bg-black bg-opacity-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md" @click.stop>
            <div class="flex items-center justify-between p-4 border-b border-gray-200">
                <h3 class="text-base font-semibold text-gray-900">Koreksi Pengajuan</h3>
                <button @click="showCorrectionForm = false" class="text-gray-400 hover:text-gray-600 rounded-full p-1 transition-colors">
            ...
            <div class="p-4 space-y-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Jenis Koreksi <span class="text-red-500">*</span></label>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <label
                            class="relative flex items-center gap-3 p-3 rounded-lg border-2 cursor-pointer transition-all"
                            :class="correctionForm.correction_type === 'submission_date'
                                ? 'border-amber-500 bg-amber-50 ring-1 ring-amber-500'
                                : 'border-gray-200 bg-white hover:border-gray-300 hover:bg-gray-50'"
                        >
                            <input v-model="correctionForm.correction_type" type="radio" value="submission_date"
                        ...
                        <label
                            class="relative flex items-center gap-3 p-3 rounded-lg border-2 cursor-pointer transition-all"
                            :class="correctionForm.correction_type === 'cancellation'
                                ? 'border-amber-500 bg-amber-50 ring-1 ring-amber-500'
                                : 'border-gray-200 bg-white hover:border-gray-300 hover:bg-gray-50'"
                        >
                            <input v-model="correctionForm.correction_type" type="radio" value="cancellation"
                                class="h-4 w-4 border-gray-300 text-amber-600 focus:ring-amber-500" />
                            <span class="text-sm font-medium text-gray-700">Pembatalan</span>
                        </label>
                    </div>
                    <p v-if="correctionErrors.correction_type" class="text-xs text-red-500 mt-1">{{ correctionErrors.correction_type }}</p>
                </div>
                ...
                <div v-if="correctionForm.correction_type === 'submission_date'">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Tanggal Mulai <span class="text-red-500">*</span></label>
                    <input v-model="correctionForm.start_date" type="date"
                        class="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400"
                        :class="{ 'border-red-400': correctionErrors.start_date }" />
                    <p v-if="correctionErrors.start_date" class="text-xs text-red-500 mt-1">{{ correctionErrors.start_date }}</p>
                </div>

                <div v-if="correctionForm.correction_type === 'submission_date'">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Tanggal Selesai <span class="text-red-500">*</span></label>
                    <input v-model="correctionForm.end_date" type="date"
                        class="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400"
                        :class="{ 'border-red-400': correctionErrors.end_date }" />
                    <p v-if="correctionErrors.end_date" class="text-xs text-red-500 mt-1">{{ correctionErrors.end_date }}</p>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">
                        Alasan Koreksi
                        <span v-if="correctionForm.correction_type === 'cancellation'" class="text-red-500">*</span>
                    </label>
                    <textarea v-model="correctionForm.correction_reason" rows="2"
                        class="w-full rounded border border-gray-300 px-3 py-2 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-amber-400"
                        :class="{ 'border-red-400': correctionErrors.correction_reason }"
                        :placeholder="correctionForm.correction_type === 'cancellation' ? 'Wajib diisi untuk pembatalan' : 'Opsional'"></textarea>
                    <p v-if="correctionErrors.correction_reason" class="text-xs text-red-500 mt-1">{{ correctionErrors.correction_reason }}</p>
                </div>

                <div v-if="correctionForm.correction_type === 'cancellation'" class="rounded-md bg-red-50 border border-red-200 p-3 text-xs text-red-700">
                    Pembatalan akan mengubah status pengajuan menjadi <span class="font-semibold">Dibatalkan</span>
                    dan menggunakan alasan koreksi sebagai keterangan pembatalan.
                </div>
            </div>
            <div class="flex gap-3 justify-end p-4 border-t border-gray-200">
                <button @click="showCorrectionForm = false"
                    class="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200 transition-colors">
                    Batal
                </button>
                <button @click="submitCorrection" :disabled="isCorrecting"
                    ...
                    <svg v-if="isCorrecting" class="w-4 h-4 mr-1 inline animate-spin" fill="none" viewBox="0 0 24 24">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" />
                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
                    </svg>
                    {{ isCorrecting ? 'Menyimpan...' : 'Simpan Koreksi' }}
                </button>
            </div>
        </div>
    </div>
...
const correctionForm = reactive({
    correction_type: 'submission_date',
    start_date: '',
    end_date: '',
    correction_reason: '',
});

// Check if user can correct dates (Superadmin or HR)
const canCorrectDate = computed(() => {
    return is('Superadmin') || is('HR');
});

// Check if user has permission to approve annual leave submissions
const canApprove = computed(() => {
    return can('submission_leave.approve');
});
...
const keterangan = ref(props.submission?.keterangan || "");
...
const isStatusApprovedOrRejected = computed(() => {
    ...
    const originalStatus = props.submission?.data?.status ?? props.submission?.status ?? 0;
...
const attachments = computed(() => {
    // Get attachment from submission data - using attachment column
    const attachment = props.submission?.data?.attachment || props.submission?.attachment;

    if (Array.isArray(attachment)) {
        // Return all attachments from the attachment column
        return attachment;
    } else if (typeof attachment === 'string') {
        // Handle single attachment as string
        return [{ file_url: attachment, file_name: attachment.split('/').pop() }];
    }
...
const imageAttachments = computed(() => {
    return attachments.value; // Display all attachments from attachment column
});
    ...
    if (newSubmission) {
        selectedStatus.value =
            typeof newSubmission.data?.status !== "undefined"
                ? newSubmission.data.status
                : (newSubmission.status ?? 0);
        keterangan.value = newSubmission.keterangan || "";
        correctionForm.start_date = normalizeDateForInput(newSubmission.data?.corrected_start_date || newSubmission.data?.start_date);
        correctionForm.end_date = normalizeDateForInput(newSubmission.data?.corrected_end_date || newSubmission.data?.end_date);
        correctionForm.correction_type = 'submission_date';
...
function getAttachmentDisplayName() {
    ...
    if (Array.isArray(attachment) && attachment.length > 0) {
        ...
        return `${attachment.length} lampiran`;
    } else if (typeof attachment === 'string') {
        // If attachment is a string, extract filename
...
function handleConfirm() {
    ...
    router.put(route('submission.annual-leave.update', props.submission?.data?.id), {
...
function submitCorrection() {
    ...
    if (!correctionForm.correction_type) {
        correctionErrors.value.correction_type = 'Jenis koreksi wajib dipilih';
        return;
    }

    if (correctionForm.correction_type === 'submission_date') {
        if (!correctionForm.start_date) {
            correctionErrors.value.start_date = 'Tanggal mulai wajib diisi';
            return;
        }
        if (!correctionForm.end_date) {
            correctionErrors.value.end_date = 'Tanggal selesai wajib diisi';
            return;
        }
        if (correctionForm.end_date < correctionForm.start_date) {
            correctionErrors.value.end_date = 'Tanggal selesai harus setelah tanggal mulai';
            return;
        }
    }

    if (correctionForm.correction_type === 'cancellation' && !correctionForm.correction_reason?.trim()) {
        correctionErrors.value.correction_reason = 'Alasan koreksi wajib diisi untuk pembatalan';
        return;
    }

    isCorrecting.value = true;

    router.patch(route('submission.annual-leave.correct', props.submission?.data?.id), {
        correction_type: correctionForm.correction_type,
        start_date: correctionForm.start_date,
        end_date: correctionForm.end_date,
        correction_reason: correctionForm.correction_reason,
    }, {
        onSuccess: () => {
            showCorrectionForm.value = false;
            correctionForm.correction_type = 'submission_date';
            correctionForm.start_date = '';
            correctionForm.end_date = '';
            correctionForm.correction_reason = '';
            emit('update-success');
        },
        onError: (errors) => {
            correctionErrors.value = errors;
        },
        onFinish: () => {
