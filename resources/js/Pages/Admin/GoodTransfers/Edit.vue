<template>
    <Head title="Ubah Pemindahan Barang" />
                    ...
                    <tbody>
                        ...
                        >
                            ...
                            >
                                ...
                                <div v-if="form.errors[`items.${i}.item_id`]" class="text-xs text-rose-600 mt-1 flex items-center gap-1">
                                    <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
                                    </svg>
                                    <span>{{ Array.isArray(form.errors[`items.${i}.item_id`]) ? form.errors[`items.${i}.item_id`][0] : form.errors[`items.${i}.item_id`] }}</span>
                                </div>
                            </td>
                            ...
                            >
                                ...
                                >
                                    <input
                                        ...
                                        :class="[
                                            ...
                                            form.errors[`items.${i}.amount`] || isQuantityExceedStock(r)
                                ...
                                <!-- Pesan error validasi untuk amount (jumlah pengiriman) -->
                                <div v-if="form.errors[`items.${i}.amount`]" class="text-xs text-rose-600 mt-1 flex items-center gap-1">
                                    <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
                                    </svg>
                                    <span>{{ Array.isArray(form.errors[`items.${i}.amount`]) ? form.errors[`items.${i}.amount`][0] : form.errors[`items.${i}.amount`] }}</span>
                                </div>
                                <!-- Real-time validation for stock exceed -->
                                <div v-else-if="isQuantityExceedStock(r)" class="text-xs text-rose-600 mt-1 flex items-center gap-1">
                                    <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
                                    </svg>
                            ...
                            >
                                ...
                                <div v-if="form.errors[`items.${i}.note`]" class="text-xs text-rose-600 mt-1 flex items-center gap-1">
                                    <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
                                    </svg>
                                    <span>{{ Array.isArray(form.errors[`items.${i}.note`]) ? form.errors[`items.${i}.note`][0] : form.errors[`items.${i}.note`] }}</span>
                                </div>
                            </td>
                            <td
                ...
                <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                        ...
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-gray-100" id="modal-title">
                                    Konfirmasi Perubahan Pemindahan Barang
                                </h3>
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500 dark:text-gray-400">
                                        Apakah Anda yakin ingin menyimpan perubahan pada pemindahan barang ini?
                                    </p>
                                    <div class="mt-4 p-3 bg-gray-50 dark:bg-gray-700 rounded-md">
                                        <p class="text-sm font-medium text-gray-900 dark:text-gray-100 mb-2">Detail Perubahan:</p>
                                        <ul class="text-sm text-gray-600 dark:text-gray-300 space-y-1">
                                            <li v-for="item in items.filter(item => item.item_id)" :key="item.uid">
                                                • {{ item.name }} - {{ item.amount }} {{ item.unit?.short_name || '' }}
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                    ...
                    <div class="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                        <button @click="confirmSave" type="button"
                            ...
                            {{ form.processing ? 'Menyimpan...' : 'Ya, Simpan Perubahan' }}
                        </button>
...
// Computed property to get available options for each row (excluding already selected items)
...
const items = ref(
    props.items?.map(item => {
        ...
        return reactive({
            ...
            amount: item.amount || item.quantity_transferred || 1,
            code: item.item_code || "",
            name: item.item_name || "",
...
const form = useForm({
    from_branch_id: props.goodTransfer.from_branch_id ?? null,
    to_branch_id: props.goodTransfer.to_branch_id ?? null,
    employee_id: props.goodTransfer.employee_id ?? null,
    transferred_at: props.goodTransfer.transferred_at ?? '',
    purpose: props.goodTransfer.purpose ?? '',
...
function saveTransfer() {
    ...
    const hasInvalidItems = items.value.some((r, i) => {
        ...
        if (!r.amount || r.amount <= 0) {
            form.setError(`items.${i}.amount`, 'Jumlah pengiriman harus lebih dari 0');
            return true;
        }
        if (r.amount > r.stock) {
            form.setError(`items.${i}.amount`, `Jumlah pengiriman tidak boleh melebihi stok tersedia (${r.stock} ${r.unit?.short_name || ''})`);
            return true;
        }
        return false;
    });
    ...
    form.items = items.value.map((r, i) => ({
        uid: r.uid,
    ...
    form.put(route("good-transfers.update", props.goodTransfer.id));
}
