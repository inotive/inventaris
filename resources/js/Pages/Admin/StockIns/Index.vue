    ...
    <div class="flex flex-col h-full gap-3 px-3 overflow-hidden">
        ...
        <div class="flex flex-col overflow-hidden bg-white border border-gray-200 rounded-lg">
            <div class="flex flex-col h-16 gap-2 px-8 sm:flex-row sm:items-center sm:justify-between">
                <div class="font-bold text-gray-700 md:text-xl">Daftar Stok Masuk</div>
            ...
            <div class="overflow-auto overflow-x-auto" data-simplebar>
                <table class="min-w-full text-sm table-fixed">
                    ...
                    <thead>
                        <tr>
                            <th class="py-2.5 bg-gray-100 border-y">
                                <div class="px-3 font-medium">No.</div>
                            </th>
                            <th class="py-2.5 bg-gray-100 border-y">
                            ...
                            <th class="py-2.5 bg-gray-100 border-y">
                                <button @click="changeSort('created_at')" class="w-full px-3 font-medium text-center text-gray-600">
                                    Tanggal Masuk
                    ...
                    <tbody>
                        <tr v-if="!itemStocks.data || itemStocks.data.length === 0">
                            <td colspan="10" class="py-8 text-center text-gray-500">Stok masuk tidak ditemukan.</td>
                        </tr>
                            ...
                            <template v-for="(rows, gkey) in groupedData" :key="gkey">
                                ...
                                <template v-if="!isCollapsed(gkey)">
                                    <tr v-for="(row, idx) in rows" :key="row.id" class="border-b border-gray-200">
                                        ...
                                        <td class="p-3 font-semibold text-center text-green-400">+ {{ row.amount }} {{ row.item.unit?.name }}</td>
                                        ...
                                        <td class="p-3 text-center">
                                            <div class="flex items-center justify-center gap-3">
                                                <button v-if="can('stock_in.edit')" @click="openEditModal(row)" class="text-yellow-500 hover:text-yellow-600" title="Edit">
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                                    </svg>
                                                </button>

                                                <button v-if="can('stock_in.delete')" @click="confirmDelete(row.id)" class="text-red-500 hover:text-red-600" title="Hapus">
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <!-- Pagination (pakai link dari paginator Laravel) -->
            ...
            <Modal
                :show="isModalOpen"
                :title="isEditMode ? 'Edit Stok Masuk' : 'Tambah Stok Masuk'"
                :confirmText="isSaving ? 'Menyimpan...' : 'Simpan'"
                maxWidth="md"
                @close="onCloseModal"
                @confirm="saveStock"
            >
                <!-- Loading bar di dalam modal saat submit -->
                ...
                <div class="space-y-4 text-sm">
                    ...
                    <div>
                        ...
                        <input v-model.number="form.amount" type="number" step="0.0001" min="0" :disabled="form.processing || isSaving" class="w-full mt-1 text-sm border-gray-400 rounded" placeholder="Masukkan jumlah" />
                    ...
                    <div>
                        ...
                        <textarea v-model="form.note" rows="3" :disabled="form.processing || isSaving" class="w-full mt-1 text-sm border-gray-400 rounded" placeholder="Catatan (opsional)"></textarea>
...
import SelectBrng from "@/Components/form/SelectBrng.vue";
...
const breadcrumbs = [{ label: "Penyimpanan" }, { label: "Stok Masuk" }];
...
function keyOf(row) {
    switch (effectiveGroup.value) {
        case "created_at": return row.created_at ?? "Tanggal Masuk";
...
function groupLabel(key) {
    switch (effectiveGroup.value) {
        case "created_at": return `Tanggal Masuk: ${key}`;
...
function fetchStocks({ sortBy = props.sortBy, sortDirection = props.sortDirection } = {}) {
    router.get(route("stock-in.index"), {
        ...filtersLocal.value,
...
function clearFilters() {
    filtersLocal.value = { q: "", branch_id: null, item_id: null, date_from: "", date_to: "", status: "" };
    groupByLocal.value = "";
    fetchStocks({});
}
...
const isSaving    = ref(false); // loading state custom (tambahan di luar form.processing)
...
function onCloseModal() {
    if (form.processing || isSaving.value) return; // cegah tutup saat submit
...
function doSubmitPut() {
    lastAction = doSubmitPut; // untuk retry
    form.put(route("stock-in.update", form.id), {
        ...
        onError: (errors) => {
            // Jika tidak ada error field (mis. network error), tampilkan modal error umum
...
function doSubmitPost() {
    lastAction = doSubmitPost; // untuk retry
    form.post(route("stock-in.store"), {
        onStart: () => { isSaving.value = true; },
        onSuccess: () => {
            isModalOpen.value = false;
...
function confirmDelete(id) {
    if (!can('stock_in.delete')) return;
    if (confirm("Apakah Anda yakin ingin menghapus data ini?")) {
        router.delete(route("stock-in.destroy", id), {
